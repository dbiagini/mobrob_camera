...
void fuse_withGyro(float* in_pest, float* in_rate, unsigned short in_dt, float* o_est){
      ...
      if(in_pest && in_rate && o_est){
        ...
        for(i=0;i<3;i++)
        {
            angle[i]= PrjR[i]+ in_rate[i]* (in_dt/CT); ///update the state in seconds
    ...
    if(MAG_testConnection()){
     ...
     timerT0=TCNT1;
      ...
      while(1){     
        ...
        dT=abs(timerT1>timerT0?timerT1-timerT0:((0xFFFF-timerT0)+timerT1));
        //TCNT1=0x0;
        timerT0=TCNT1;
        //timerT0=timerT1; 
